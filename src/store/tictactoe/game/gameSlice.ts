...
import {
	...
	GameStateMetaI,
	MoveActionPayloadI,
} from 'store/tictactoe/game/types';
	...
	reducers: {
		...
		togglePause(container: GameStateContainerI) {
			...
			const newStatus: GameStatus =
				status === GameStatus.Paused ? GameStatus.Running : GameStatus.Paused;
		...
		finishSavingGame(container: GameStateContainerI, action: PayloadAction<boolean>) {
			const newStatus = action.payload ? GameStatus.Waiting : GameStatus.Stopped;
			updateStatusGame(container, newStatus);
		},
...
export const { goToMove, makeMove, restoreGame, togglePause, updateGameDuration, newGame } =
	gameSlice.actions;
