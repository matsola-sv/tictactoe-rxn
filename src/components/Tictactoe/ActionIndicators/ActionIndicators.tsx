...
const ActionIndicators: FC = () => {
	...
	const statusGame: GameStatus = useTypedSelector((state) => state.t3game.state.status);
	...
	const hasActions: boolean = indicators.some((indicator) => indicator.isActive);
		...
		<div className='action-indicators'>
			...
			{indicators.map((indicator) =>
				indicator.isActive ? (
					<div
						key={indicator.id}
						className={`action-indicator ${indicator.className}`}
					>
						<FontAwesomeIcon
							icon={indicator.icon}
							className='icon'
